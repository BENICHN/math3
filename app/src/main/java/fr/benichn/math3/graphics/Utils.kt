...
object Utils {
    fun getTextPathAndSize(
        ...
        paint.getTextPath(
            ...
            text.length,
            0f,
            0f - (paint.fontMetrics.ascent + paint.fontMetrics.descent) / 2,
            res
        )
    ...
    fun sumOfRects(rects: Iterable<RectF>): RectF =
        rects.fold(RectF(Float.NaN, Float.NaN, Float.NaN, Float.NaN)) { acc, r ->
            RectF(
